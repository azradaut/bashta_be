...
public interface IWateringEventRepository
{
    ...
    Task<int> CountManualNonSkippedByPotIdSinceAsync(int potId, DateTime sinceUtc);
    Task<List<WateringEvent>> GetRecentCompletedByPotIdAsync(int potId, int limit = 5);
    Task<WateringEvent?> GetPendingManualByPotIdAsync(int potId);
    Task<WateringEvent?> GetByIdAsync(int id);
    Task<WateringEvent> CreateAsync(WateringEvent wateringEvent);
    Task<WateringEvent> UpdateAsync(WateringEvent wateringEvent);
}
